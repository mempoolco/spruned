...
from spruned.reactors.reactor_types import FetchHeadersResponse
...
class HeadersReactor:
    ...
    def _set_best_chain(self, best_chain: typing.List[BlockHeader]):
        assert not self._best_chain
        assert best_chain
        for i, header in enumerate(best_chain):
            self._best_chain.append(header)
            self._best_chain_height_index[header.hash] = header.height

    def _append_to_best_chain(self, header: BlockHeader):
        self._best_chain.append(header)
        self.interface.set_local_current_header(header)
        self._best_chain_height_index[header.hash] = header.height
        self._best_chain_height_index.pop(self._best_chain.pop(0).hash)
    ...
    async def start(self):
        ...
        best_chain = await self.repo.get_headers(header.hash)
    ...
    async def _fetch_headers_loop(self):
        ...
        try:
            ...
            if self.initial_headers_download and \
                    self._best_chain[-1].height >= self.interface.get_current_peers_best_height():
    ...
    async def _check_headers_with_best_chain(
            self, connection: P2PConnection, headers: typing.List[BlockHeader]
    ) -> typing.List[BlockHeader]:
        start_from_height = self._get_height_for_hash(headers[0].prev_block_hash)
        if start_from_height is not None \
                and start_from_height + len(headers) + 1 > self._best_chain[-1].height:
            headers[0].height = start_from_height + 1
    ...
    async def _evaluate_received_headers(self, headers: typing.List[BlockHeader]) \
            -> typing.Optional[typing.List[BlockHeader]]:
        start_height = self._best_chain[0].height
        pos = headers[0].height - start_height
        assert pos >= 0
        ...
        if not new_headers:
            return
        new_headers[0].height = headers[0].height + pos_after_match
        return new_headers

    async def _evaluate_headers_consensus(self, headers: typing.List[BlockHeader]) -> typing.List[BlockHeader]:
        """
        ensure:
        - chain-link is ok
        - pow is ok
    ...
    async def on_headers(
        ...
        try:
            headers = list(
                map(
                    lambda h: BlockHeader(
                        hash=blockheader_to_blockhash(h[0].as_bin()),
                        data=h[0].as_bin()
                    ),
                    headers
                )
    ...
    async def _handle_new_headers(self, new_headers: typing.List[BlockHeader]):
        Logger.p2p.debug('Received %s new headers' % len(new_headers))
        new_headers = await self._evaluate_headers_consensus(new_headers)
    ...
    def _get_headers_for_agreement(self, headers: typing.List[BlockHeader]) -> typing.List[BlockHeader]:
        """
        Return a chunk of headers from the known best_chain, to be checked for agreement between multiple peers.
        """
        assert headers[0].prev_block_hash == self._best_chain[-1].hash
        for i, h in enumerate(headers, start=1):
            h.height = self._best_chain[-1].height + i
    ...
    async def _fetch_headers(self):
        ...
        self._last_connection = await self.interface.get_headers_after_hash(
            *map(lambda h: h.hash, chain)
        )

    @staticmethod
    async def _fetch_header_blocking(connection, responses: FetchHeadersResponse, headers: typing.List[BlockHeader]):
        try:
            h = await connection.fetch_headers_blocking(
                *map(lambda x: x.hash, headers[:-1]),
                stop_at_hash=headers[-1].hash
            )
    ...
    def _evaluate_agreement_for_headers(
            _headers: typing.List[BlockHeader],
            _responses: FetchHeadersResponse,
            origin_peer: P2PConnection
    ):
        """
        ...
        for x in range(0, len(_headers[2:])):
            ...
            for c, r in _responses.items():
                ...
                block_height = _headers[2+x].height
    ...
    async def ensure_agreement_for_headers(
        ...
        start = time.time()
        responses = FetchHeadersResponse()
