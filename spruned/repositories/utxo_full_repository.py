import asyncio
from concurrent.futures.thread import ThreadPoolExecutor
from enum import Enum

import plyvel
import typing

from spruned.reactors.reactor_types import DeserializedBlock
from spruned.repositories import exceptions
from spruned.repositories.utxo_diskdb import UTXODiskDB


class DBPrefix(Enum):
    DB_VERSION = 1
    CURRENT_BLOCK_HEIGHT = 2
    SHARD = 3
    UTXO = 4
    UTXO_REVERTS = 5


class UTXOXOFullRepository:
    def __init__(
        self,
        leveldb: plyvel.DB,
        disk_db: UTXODiskDB
    ):
        self.leveldb = leveldb
        self.loop = asyncio.get_event_loop()
        self.executor = ThreadPoolExecutor(max_workers=32)
        self._best_header = None
        self._disk_db = disk_db
        self._safe_height = 0

    def set_safe_height(self, safe_height: int):
        self._safe_height = safe_height

    @staticmethod
    def _get_db_key(prefix: DBPrefix, name: bytes = b''):
        assert isinstance(name, bytes)
        return b'%s%s' % (int.to_bytes(prefix.value, 2, "big"), name)

    async def process_block(self, block: DeserializedBlock):
        return await self.loop.run_in_executor(
            self.executor,
            self._process_block,
            block
        )

    def _process_block(self, block: DeserializedBlock):
        revert_utxo = []
        session = self.leveldb.write_batch()
        height_in_bytes = block.block.height.to_bytes(4, 'little')
        for tx in block.deserialized['txs']:
            for vin in tx['ins']:
                revert_utxo.append(
                    self._spend_utxo(
                        session,
                        vin,
                        safe=block.block.height < self._safe_height
                    )
                )
            for i, vout in enumerate(tx['outs']):
                self._add_utxo(session, vout, tx, i, height_in_bytes, tx['gen'])
        session.put(
            self._get_db_key(DBPrefix.CURRENT_BLOCK_HEIGHT),
            block.block.height.to_bytes(4, 'little')
        )
        self._disk_db.save_revert_state(block.block.hash, revert_utxo)
        session.write()
        return True

    async def revert_block(self, block: DeserializedBlock):
        return await self.loop.run_in_executor(
            self.executor,
            self._revert_block,
            block,
        )

    def _revert_block(self, block: DeserializedBlock) -> bool:
        raise NotImplementedError

    def _verify_utxo(self, vin: typing.Dict, utxo_key: bytes) -> bytes:
        # todo full validation
        existing = self.leveldb.get(utxo_key)
        if existing is None:
            raise exceptions.UTXOInconsistencyException('utxo %s does not exist' % utxo_key)
        return existing

    def _spend_utxo(self, session: plyvel.DB, vin, safe=True) -> bytes:
        utxo_key = self._get_db_key(DBPrefix.UTXO, vin['hash'] + vin['index'].to_bytes(4, 'little'))
        existing = None if safe else self._verify_utxo(vin, utxo_key)
        session.delete(utxo_key)
        return existing

    def _backup_utxo_for_rollback(self, session, existing: bytes, utxo_key: bytes):
        session: plyvel.DB
        session.put(
            self._get_db_key(DBPrefix.UTXO_REVERTS, utxo_key[2:]),
            existing
        )

    def _add_utxo(
            self, session: plyvel.DB,
            vout: typing.Dict,
            tx: typing.Dict,
            i: int,
            block_height: bytes,
            is_coinbase: bool
    ):
        utxo_key = self._get_db_key(DBPrefix.UTXO, tx['hash'] + i.to_bytes(4, 'little'))
        session.put(
            utxo_key,
            vout['amount'].to_bytes(8, 'little') +
            is_coinbase.to_bytes(1, 'little') +
            block_height +
            vout['script']
        )

    def get_utxo(self, txid: bytes, index: int) -> bool:
        pass
