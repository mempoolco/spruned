...
class TestElectrodInterface(unittest.TestCase):
    def setUp(self):
        ...
        self.electrum_header = {
            'height': 513526,
            'hex': '00000020fe52010fa7c798b97621508b772142dfc7b594df7a3a3200000000000000000097db2dc94e2799bcdd7259f0876467b379fe44b69342df38a8ec2f350722f9a73367a95aa38955173732b883'
        }
        self.parsed_header = {
    ...
    def test_get_header_invalid_pow(self):
        ...
        electrum_header.update({'hex': '00000020fe52010fa7c798b97621508b772142dfc7b594df7a3a320000000000000000001'
    ...
    def test_subscription_observers_pow_error(self):
        ...
        observers = []
        electrum_header = {k: v for k, v in self.electrum_header.items()}
        electrum_header.update({'hex': '00000020fe52010fa7c798b97621508b772142dfc7b594df7a3a320000000000000000009'
                                       '7db2dc94e2799bcdd7259f0876467b379fe44b69342df38a8ec2f350722f9a73'
                                       'fffffffffffffffffffffff'})  # <- target to the moon
        self.connectionpool.add_header_observer.side_effect = lambda x: observers.append(x)
        self.connectionpool.call.return_value = async_coro((peer, electrum_header))
        self.sut.add_header_subscribe_callback(callback)
